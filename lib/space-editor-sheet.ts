...
export function ownsEditorSheetHistoryState(state: unknown, token: string | null) {
...
export type EditorSheetRestoration = {
  capture: (focusTarget: FocusTargetLike | null, regions: Iterable<ScrollRegionLike>) => void;
...
export function createEditorSheetRestoration(
  ...
  const restore = (fallbackFocus: FocusTargetLike | null = null) => {
    ...
    frameId = runtime.requestAnimationFrame(() => {
      ...
      const target = focusTarget?.isConnected === false ? fallbackFocus : focusTarget;
