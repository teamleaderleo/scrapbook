...
## Connected one-kickoff continuation

The three phases above remain the durable workflow boundaries, but they do not require three separately written user instructions when ChatGPT has native image generation plus connected Google Drive and GitHub tools.

A user may instead give one kickoff request:
...
After that kickoff, continue through every available phase without asking the user to restate the next step. User-visible interruptions are reserved for a platform-required approval, a genuinely ambiguous creative decision, an unavailable generated artifact or connector, a failed import or validation, or a separate ready or merge confirmation.

Use a bounded artwork budget rather than an open-ended loop:
...
Do not spend all attempts by default. Stop when one candidate is clearly good enough. If image generation ends the current assistant response, preserve the reserved branch and signed session as continuation state and resume the remaining connector chain as soon as the generated file reference is available.

For a connector-driven import, upload the selected runtime file through Google Drive, then commit exactly one request on the reserved branch:
...
The push-triggered importer validates the owner, branch, request path, source, and derived output path; commits `public/gallery/agents/<entry-id>.webp`; and removes the request file. The assistant should then continue with the typed entry, draft pull request, and CI inspection without another explanatory user message.
