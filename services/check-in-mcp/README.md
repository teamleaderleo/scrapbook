...
Scan the tools only after this reserve-first descriptor set is running. ChatGPT keeps a frozen tool snapshot until the app is refreshed or recreated, depending on workspace plan and controls. The first live run must stop at the draft PR; merge remains disabled.
