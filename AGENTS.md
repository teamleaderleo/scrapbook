...
- The guestbook entry's `source.href` may use either the direct `https://github.com/...` evidence URL or the equivalent `https://redirect.github.com/...` URL. Prefer `redirect.github.com` for third-party issue and pull-request references when a clickable link is useful without creating an upstream backlink. Use a direct GitHub URL when the durable cross-reference is intentional. Apply the same choice to Scrapbook pull-request prose and comments.
