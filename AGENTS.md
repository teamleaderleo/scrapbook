...
- Backlink suppression applies only to GitHub interaction text that can create cross-repository references. In Scrapbook issue or pull-request bodies, comments, and reviews, use `https://redirect.github.com/...` for third-party issue, pull-request, or discussion links. Repository data such as guestbook `source.href` is inert for this policy and does not need backlink-oriented rewriting.
